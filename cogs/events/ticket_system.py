...
class TicketReasonSelect(discord.ui.Select):
    def __init__(self, ticket_cog):
        ...
        reasons = (
            ticket_cog.ticket_reasons
            if ticket_cog.ticket_reasons
            else [
                {
    ...
    async def callback(self, interaction: discord.Interaction):
        try:
            ...
            await self.ticket_cog.handle_ticket_creation_with_reason(
                interaction, reason
            )
            ...
            if channel:
                try:
                    message = await channel.fetch_message(
                        self.ticket_cog.ticket_message_id
                    )
                    ...
                    logger.warning(
                        "⚠️ Message de tickets non trouvé lors de la réinitialisation du menu."
                    )
                except Exception as e:
                    logger.error(
                        f"❌ Erreur lors de la réinitialisation du menu: {str(e)}"
                    )
        except Exception as e:
...
class TicketLogView(discord.ui.View):
    ...
    @discord.ui.button(
        ...
        emoji="📝",
        custom_id="view_ticket_content",
    )
    async def view_content_button(
        ...
        if not interaction.user.guild_permissions.administrator:
            await interaction.response.send_message(
                "❌ Vous n'avez pas la permission de voir le contenu des tickets archivés.",
                ephemeral=True,
            )
            return
        ...
        if self.ticket_id not in self.ticket_cog.archived_tickets:
            await interaction.response.send_message(
                "❌ Ce ticket n'existe plus dans les archives.", ephemeral=True
            )
            return
        ...
        owner = (
            interaction.guild.get_member(owner_id) or f"Utilisateur (ID: {owner_id})"
        )
        archived_by = (
            interaction.guild.get_member(archived_by_id)
            or f"Administrateur (ID: {archived_by_id})"
        )
        ...
        embed.add_field(
            name="Archivé le",
            value=archived_at.strftime("%d/%m/%Y à %H:%M"),
            inline=True,
        )
        ...
        if not messages:
            await interaction.followup.send(
                "❌ Aucun message trouvé dans ce ticket.", ephemeral=True
            )
            return
        ...
        for msg in messages:
            ...
            if (
                len(current_chunk) + len(line) > 1900
            ):  # Limite de Discord moins une marge
                current_chunk += "```"
...
class TicketSystem(commands.Cog, name="tickets"):
    def __init__(self, bot):
        ...
        self.active_tickets = (
            {}
        )  # {channel_id: {"owner": user_id, "config_message": message_id}}
        self.archived_tickets = (
            {}
        )  # {ticket_id: {"owner": user_id, "archived_by": user_id, "archived_at": datetime, "messages": [...], "close_reason": str}}
        ...
        self.color = (
            EmbedManager.get_default_color()
        )  # Utiliser la couleur par défaut du gestionnaire d'embeds
        self.load_config()
    ...
    def load_config(self):
        ...
        try:
            ...
            logger.error(
                f"❌ Erreur lors du chargement de la configuration des tickets: {str(e)}"
            )
            self.ticket_category_id = None
            self.create_channel_id = None
            self.log_channel_id = None
        ...
        try:
            if os.path.exists(self.archive_file):
                with open(self.archive_file, "r", encoding="utf-8") as f:
                    ...
                    logger.info(
                        f"✅ {len(self.archived_tickets)} tickets archivés chargés"
                    )
            else:
                self.archived_tickets = {}
    ...
    def save_config(self):
        ...
        try:
            ...
            logger.error(
                f"❌ Erreur lors de la sauvegarde de la configuration des tickets: {str(e)}"
            )
    ...
    def save_archived_tickets(self):
        ...
        try:
            ...
            logger.error(
                f"❌ Erreur lors de la sauvegarde des tickets archivés: {str(e)}"
            )
    ...
    async def refresh_ticket_message_on_startup(self):
        ...
        try:
            ...
            if not channel:
                logger.warning(
                    f"⚠️ Canal de tickets introuvable (ID: {self.create_channel_id})"
                )
                return False
            ...
            logger.info(
                f"✅ Couleur du système de tickets mise à jour: {hex(self.color)}"
            )
            ...
            logger.error(
                f"❌ Erreur lors du rafraîchissement du message des tickets: {str(e)}"
            )
            return False
    ...
    async def setup_tickets(self, ctx):
        ...
        try:
            ...
            create_channel = discord.utils.get(
                guild.text_channels, name=create_channel_name
            )
            if not create_channel:
                create_channel = await guild.create_text_channel(
                    create_channel_name, category=category
                )
            ...
            logs_channel = discord.utils.get(
                guild.text_channels, name=logs_channel_name, category=category
            )
            if not logs_channel:
                ...
                for role in guild.roles:
                    if role.permissions.administrator:
                        logs_overwrites[role] = discord.PermissionOverwrite(
                            view_channel=True, send_messages=True
                        )
                ...
                await ctx.send(
                    f"📋 Canal de logs de tickets créé: {logs_channel.mention}"
                )
            else:
                self.log_channel_id = logs_channel.id
            ...
            ticket_embed = self.create_ticket_embed()
            ticket_message = await create_channel.send(
                embed=ticket_embed, view=TicketCreationView(self)
            )
            ...
            embed.add_field(
                name="Canal de création", value=f"{create_channel.mention}", inline=True
            )
            embed.add_field(
                name="Canal de logs", value=f"{logs_channel.mention}", inline=True
            )
            embed.set_footer(
    ...
    async def setup_ticket_reasons(self, ctx):
        ...
        await ctx.send(
            "🔄 Configuration des raisons de tickets. Envoyez `annuler` pour annuler à tout moment.\n\n"
        ...
        try:
            ...
            try:
                ...
                if num_reasons < 1 or num_reasons > 10:
                    await ctx.send(
                        "❌ Le nombre doit être entre 1 et 10. Configuration annulée."
                    )
                    return
            except ValueError:
                await ctx.send(
                    "❌ Veuillez entrer un nombre valide. Configuration annulée."
                )
                return
            ...
            for i in range(1, num_reasons + 1):
                await ctx.send(
                    f"📝 Configuration de la raison #{i}:\n\n"
                    f"Veuillez entrer le libellé (ex: 'Assistance technique'):"
                )

                label_resp = await self.bot.wait_for(
                ...
                emoji_resp = await self.bot.wait_for(
                    "message", check=check, timeout=60.0
                )
                ...
                desc_resp = await self.bot.wait_for(
                    "message", check=check, timeout=60.0
                )
                ...
                if self.ticket_message_id:
                    ...
                    if channel:
                        try:
                            ...
                            await ctx.send(
                                "✅ Menu de tickets mis à jour avec les nouvelles raisons!"
                            )
                        except:
                            await ctx.send(
                                "⚠️ Message de tickets introuvable. Utilisez !ticketsetup pour recréer le message."
                            )
            except Exception as e:
                logger.error(
                    f"❌ Erreur lors de la mise à jour du menu de tickets: {str(e)}"
                )
                await ctx.send(
                    "⚠️ Les raisons ont été enregistrées mais le menu n'a pas pu être mis à jour."
                )

            await ctx.send(
                f"✅ Configuration terminée! {len(new_reasons)} raisons de tickets configurées."
            )
    ...
    async def refresh_ticket_menu(self, ctx):
        ...
        try:
            ...
            embed.add_field(
                name="ID du message", value=self.ticket_message_id, inline=True
            )
    ...
    async def delete_ticket_command(self, ctx):
        ...
        try:
            ...
            if log_channel:
                ...
                log_embed.add_field(
                    name="Raison", value="Suppression directe", inline=True
                )
                ...
                log_embed.set_footer(
                    text=f"Supprimé par: {ctx.author.name} (ID: {ctx.author.id})"
                )
    ...
    async def rename_ticket_command(self, ctx, *, new_name: str):
        ...
        clean_name = "".join(
            c for c in new_name.strip().lower() if c.isalnum() or c == "-"
        )
    ...
    async def handle_ticket_close_reaction(self, channel, member, is_owner):
        ...
        try:
            ...
            confirm_embed.set_footer(
                text="Réagissez avec ✅ pour confirmer ou ❌ pour annuler."
            )
            ...
            try:
                ...
                if str(reaction.emoji) == "✅":
                    ...
                    await channel.send(
                        "❌ Fermeture du ticket annulée.", delete_after=10
                    )

            except asyncio.TimeoutError:
                await channel.send(
                    "❌ Confirmation expirée. Fermeture du ticket annulée.",
                    delete_after=10,
                )

        except asyncio.TimeoutError:
            await channel.send(
                "❌ Temps écoulé. Veuillez réessayer si vous souhaitez toujours fermer ce ticket.",
                delete_after=10,
            )
    ...
    async def archive_ticket_with_reason(self, channel, member, reason):
        ...
        try:
            ...
            await channel.send(
                f"🗃️ Ce ticket va être archivé dans 5 secondes...\n**Raison:** {reason}\n**ID:** {ticket_id}"
            )
            await asyncio.sleep(5)
            ...
            if archive_category:
                try:
                    ...
                    for role in guild.roles:
                        if role.permissions.administrator:
                            archive_overwrites[role] = discord.PermissionOverwrite(
                                view_channel=True
                            )
                    ...
                    await channel.send(
                        f"📁 Ce ticket a été archivé pour raison: **{reason}**"
                    )

                except discord.Forbidden:
                    await channel.send(
                        "❌ Je n'ai pas les permissions pour archiver ce ticket. Il sera supprimé."
                    )
    ...
    async def on_raw_reaction_add(self, payload):
        ...
        if (
            hasattr(self, "ticket_message_id")
            and payload.message_id == self.ticket_message_id
        ):
            ...
            if payload.message_id == ticket_data.get("config_message"):
                ...
                if str(payload.emoji) == "🔒":
                    ...
                    if is_owner or is_admin:
                        await self.handle_ticket_close_reaction(
                            channel, member, is_owner
                        )
                    else:
                        await channel.send(
                            f"{member.mention} Seul le créateur du ticket ou un administrateur peut fermer ce ticket.",
                            delete_after=10,
                        )
            ...
            if hasattr(self, "create_channel_id") and self.create_channel_id:
                ...
                if channel and hasattr(self, "ticket_message_id"):
                    try:
                        ...
                        logger.error(
                            f"❌ Erreur lors de la mise à jour de la couleur: {str(e)}"
                        )
        except Exception as e:
