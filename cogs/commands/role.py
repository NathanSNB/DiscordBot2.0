...
class RolesManagementCog(commands.Cog, name="RoleManager"):
        ...
        try:
            if os.path.exists(self.channel_config_file):
                with open(self.channel_config_file, "r") as f:
                    config = json.load(f)
                    self.default_channel_id = config.get(
                        "channel_id", self.default_channel_id
                    )
                logger.info(
                    f"📝 Configuration du canal chargée: {self.default_channel_id}"
                )
        except Exception as e:
    ...
    async def role_menu_command(self, ctx):
        ...
        try:
            if not self.roles_data:
                await ctx.send(
                    "❌ Aucun rôle configuré. Utilisez !roleadd pour en ajouter."
                )
                return
    ...
    async def refreshroles_command(self, ctx):
        ...
        try:
            ...
            if not channel:
                await ctx.send(
                    f"❌ Canal avec ID {self.default_channel_id} non trouvé. Utilisez !setrole pour configurer un canal valide."
                )
                return
            ...
            try:
                async for message in channel.history(limit=10):
                    if (
                        ...
                        and message.embeds
                        and "Choisissez vos rôles" in message.embeds[0].title
                    ):
    ...
    @commands.has_permissions(administrator=True)
    async def configrole_command(
        self, ctx, role: discord.Role, emoji: str = "🔹", *, description: str = ""
    ):
        ...
        logger.info(
            f"✅ Rôle {role.name} configuré par {ctx.author} avec emoji {emoji}"
        )
